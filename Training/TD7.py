...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, zs_dim=512, hdim=512, activ=F.relu):
...
class Encoder(nn.Module):
    def __init__(self, state_dim, action_dim, zs_dim=512, hdim=512, activ=F.elu):
...
class Agent(object):
    def __init__(self, state_dim, action_dim, max_action, args, hp=Hyperparameters()):
        ...
        self.model_path = self.save_dir + '/FloatingFarmYaw'
        if not os.path.exists(self.model_path):
            os.mkdir(self.model_path)

        # load model
        model_actor_name = self.model_path + '/' + args.load_name + '_actor_params.pkl'
        if os.path.exists(model_actor_name) and args.load_model:
            ...
            print('Agent successfully loaded critic_network: {}'.format(self.model_path + '/' + args.load_name
                                                                        + '_critic_params.pkl'))
            print('Agent successfully loaded encoder_network: {}'.format(self.model_path + '/' + args.load_name
                                                                         + '_encoder_params.pkl'))
    ...
    def train(self):
        ...
        with torch.no_grad():
            ...
            Q_target = \
                self.critic_target(next_state, next_action, fixed_target_zsa, fixed_target_zs).min(1, keepdim=True)[0]
